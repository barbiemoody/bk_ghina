...
function getDaftarPeriksaByDokter($id_dokter)
{
    global $conn;
...
if (isset($_SESSION['id_dokter'])) {
    $id_dokter = $_SESSION['id_dokter']; // ID dokter dari sesi login
    $daftarPasien = getDaftarPeriksaByDokter($id_dokter);
    foreach ($daftarPasien as $pasien) {
        echo "Nama Pasien: " . $pasien['nama_pasien'] . "<br>";
        echo "Keluhan: " . $pasien['keluhan'] . "<br>";
        echo "No Antrian: " . $pasien['no_antrian'] . "<br><br>";
    }
} else {
    echo "Dokter belum login.";
}
...
function TambahDetailPeriksa($data) {
    ...
    $tgl_periksa = htmlspecialchars(trim($data["tgl_periksa"]));
    $catatan = htmlspecialchars(trim($data["catatan"]));

    // Check for empty required fields
    if (empty($tgl_periksa) || empty($catatan)) {
        return [
            "status" => false,
            "message" => "Tanggal periksa dan catatan tidak boleh kosong."
        ];
    }

    // Prepare the SQL query
    $query = "INSERT INTO detail_periksa (tgl_periksa, catatan)
              VALUES ('$tgl_periksa', '$catatan')";

    // Execute the query and handle potential errors
    if (mysqli_query($conn, $query)) {
        return [
            "status" => true,
            "message" => "Detail periksa berhasil ditambahkan."
        ];
    } else {
        return [
            "status" => false,
            "message" => "Gagal menambahkan detail periksa: " . mysqli_error($conn)
        ];
    }
}
